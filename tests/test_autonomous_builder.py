...
def test_two_week_campaign_duration_is_preserved() -> None:
    parsed = parse_autonomous_request(
        "Crée et lance une campagne Autonomous pendant deux semaines".replace("deux", "2")
    )

    assert parsed is not None
    assert parsed["max_duration_seconds"] == 14 * 24 * 3600
